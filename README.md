...
Logging is widely used in industrial practice to record software runtime behaviors and to assist developers in tasks such as error tracing and anomaly detection. However, deciding where to place logging statements remains challenging, especially in large-scale software systems, where both insufficient and excessive logging can negatively affect system observability and maintenance cost. Existing approaches mainly recommend logging locations within individual functions and pay limited attention to inter-function relationships and heterogeneous feature fusion. To address these limitations, we first conduct an empirical study on 99 projects to identify the factors that may influence developers’ logging decisions. Based on the findings, we propose GraphLog, a novel logging recommendation approach that leverages a block dependency graph to model nesting and invocation relationships among code blocks. On top of this graph, GraphLog learns graph-aware representations of code blocks and integrates them with syntactic and method-level thematic features within a unified deep learning framework for logging recommendation. In this way, GraphLog captures local code content, method intent, and structural context in a coordinated manner. We implement GraphLog as an Eclipse plugin and construct a dataset containing 91,700 samples from seven real-world Java projects. Experimental results show that GraphLog achieves an F1 score of 77.23% for logging recommendation, outperforming existing approaches by 2.64%. These results demonstrate the effectiveness of combining block dependency modeling with multi-feature fusion for automated logging recommendation.
